 ...
 */
function catchRenderTarget(renderer: WebGLRenderer, target: WebGLRenderTarget) {
 ...
 * @return {string} dataURL
 */
function catchTexture(texture: Texture, renderer: WebGLRenderer, target = defaultTarget) {
  (quad.material as ShaderMaterial).uniforms.tDiffuse.value = texture;
...
function printfImage(name = " ", dateURL: string) {
  console.log(
    `%c ${name}`,
...
function printTexture(name: string, texture: Texture, renderer: WebGLRenderer, target = defaultTarget) {
  printfImage(name, catchTexture(texture, renderer, target));
}

function printRenderTarget(name: string, renderer: WebGLRenderer, target: WebGLRenderTarget) {
